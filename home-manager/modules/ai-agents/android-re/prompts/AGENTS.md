...
2. **`repro_steps`** — exact steps, Frida command, API call, or code path.
   Copy-paste reproducible. Not "see analysis" — the actual steps.
3. **`evidence_path`** — path to screenshot, captured output, Frida log, or script.
4. **`confidence`** — a real score (0.0-1.0), never leave at 0.0 default.
5. **FINDINGS.md narrative** — a markdown section with code references,
   dataflow analysis, and remediation. Not a one-liner.

**Anti-pattern (DO NOT DO THIS):** Running static analysis or dynamic testing,
finding a batch of issues, stuffing them into the DB with `confidence=0.0` and
empty description/evidence fields, then moving on. This is the same as not
writing them at all — context will be lost and the next session starts from
scratch.
