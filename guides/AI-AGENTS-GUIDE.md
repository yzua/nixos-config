...
| **agentmemory (addon)**   | `npx`      | shared MCP + `agentmemory.service`               | MCP/API  |
| **Herdr (multiplexer)**   | `herdr`    | `~/.config/herdr/config.toml` + agent hooks      | TOML/hooks |
...
## Herdr Multiplexer Usage (All Agents)

Herdr is enabled by `programs.aiAgents.herdr.enable` and installed from the pinned upstream flake `github:ogulcancelik/herdr/v0.6.2`.

Generated/runtime pieces:

- Config: `~/.config/herdr/config.toml`, generated by Home Manager.
- Package: `inputs.herdr.packages.<system>.default`.
- Direct integrations: Claude Code, Codex, and OMP are installed by activation using `herdr integration install`.
- OpenCode integration: Home Manager copies Herdr's upstream plugin into every OpenCode profile under `~/.config/opencode-*/plugins/herdr-agent-state.js` and `~/.config/opencode/plugins/herdr-agent-state.js`.
- Skill: upstream `SKILL.md` is installed into Claude, Codex, Gemini, and every OpenCode profile so agents can operate Herdr when `HERDR_ENV=1`.

Launch paths:

```bash
hd              # alias for herdr
herdr           # direct upstream launcher
zh              # zellij-herdr, opens the generated Herdr layout in Zellij
zellij -l herdr # direct layout launch
```

Inside Zellij, use `Alt a` and pick `Herdr` from the agent panel. The generated Herdr config also binds `prefix+a` to `ai-agent-launcher`, `prefix+shift+a` to `ai-agent-inventory`, and `prefix+shift+l` to `ai-agent-dashboard`.

Herdr-aware `aip` behavior:

- Inside Herdr (`HERDR_ENV=1`), `aip cl oc gem "prompt"` creates a Herdr tab, splits panes with Herdr's API, names each pane, and runs each agent command.
- Outside Herdr, the same command keeps the existing Zellij generated-layout behavior.
...
hdlogs    # tail Herdr logs
hdstop    # stop the Herdr server
```

Upstream notes:

- Herdr has direct integrations for `pi`, `omp`, `claude`, `codex`, `opencode`, and `hermes`; this repo enables the locally used targets.
- Gemini is detected by Herdr, but upstream does not provide a native Gemini hook installer yet.
- Ghostty Shift+Enter is intentionally left unbound so Herdr and nested agent TUIs can distinguish modified Enter input.

## agentmemory Usage (All Agents)

agentmemory is enabled by `programs.aiAgents.agentmemory.enable` and exposed through the shared MCP registry, so the same memory backend is available to Claude Code, OpenCode, Gemini CLI, Codex, and OMP.

Runtime pieces:

- Server: user service `agentmemory.service`, started by Home Manager.
- Engine: pinned `iii` runtime v0.11.2 from the upstream Linux release archive.
- API: `http://localhost:3111`
- Viewer: `http://localhost:3113`
- MCP shim: `bunx --silent @agentmemory/mcp@0.9.16` with `AGENTMEMORY_URL=http://localhost:3111`

Generated agent wiring:

| Agent       | Integration in this repo                         | Why this path |
| ----------- | ------------------------------------------------ | ------------- |
| Claude Code | `~/.mcp.json` shared `mcpServers.agentmemory`    | Home Manager owns MCP config; avoids plugin rewrites to managed files. |
| OpenCode    | Every profile gets top-level `mcp.agentmemory`   | Matches OpenCode's array command schema and keeps all profile variants aligned. |
| Gemini CLI  | `~/.gemini/settings.json` `mcpServers` + allowed | Gemini has an explicit MCP allow-list, so `agentmemory` is allowed there too. |
| Codex       | `~/.codex/config.toml` `[mcp_servers.agentmemory]` | Works for all profiles and custom agents from the generated TOML. |
| OMP         | `~/.omp/agent/mcp.json` shared `mcpServers`      | Uses the same shared transform as other `mcpServers` clients. |

Operational aliases:

```bash
mem-start   # start agentmemory.service
mem-stop    # stop agentmemory.service
mem-status  # inspect service logs/status
mem-health  # curl the health endpoint
mem-view    # open the live memory viewer
```

Effective use:

1. Keep it running for normal coding sessions: `mem-start`, then use any agent normally.
2. Ask agents to search memory before re-discovering project context: "use agentmemory to recall prior decisions about auth/rate limiting".
3. Save durable decisions explicitly when they matter: "remember that this repo treats `just check` as eval-only, not a full deploy validation".
4. Prefer facts, decisions, and workflow outcomes over noisy transcript dumps. Good memories name the repo, file paths, commands, and the reason behind a decision.
5. Use the viewer when memory quality looks wrong. Delete or supersede stale facts instead of letting old architecture notes compete with current code.
6. Keep secrets out of prompts. Upstream strips common secrets, but this repo still treats agent logs and memory as sensitive local state.

Upstream supports richer native plugins for Claude Code and Codex that add lifecycle hooks and slash commands. This config intentionally uses the shared MCP integration as the declarative baseline because Home Manager regenerates Claude and Codex config files. Install the native plugins manually only if you want automatic hook capture beyond MCP and are comfortable with plugin-managed changes:
...
- `home-manager/modules/terminal/ghostty.nix`, `home-manager/modules/terminal/zsh/`, and `home-manager/modules/terminal/zellij/` — Herdr launch and terminal integration paths
- `home-manager/modules/niri/binds.nix` — compositor shortcuts referenced in workflow examples
- Herdr upstream `v0.6.2` docs and source — configuration, CLI, integrations, and Ghostty Shift+Enter guidance
...
| Herdr             | <https://herdr.dev/docs>                                  | <https://github.com/ogulcancelik/herdr>                 |
...
| agentmemory   | <https://github.com/rohitg00/agentmemory#readme>           | <https://github.com/rohitg00/agentmemory> |
...
| `hd`                           | `herdr`                                                            | Herdr terminal-native multiplexer for all agent sessions                                         |
| `hds`                          | `herdr status`                                                     | Herdr session/server/client status                                                              |
| `hdi`                          | `herdr integration status`                                         | Herdr integration health                                                                         |
| `hdagents`                     | `herdr agent list`                                                 | List detected/reported Herdr agents                                                             |
| `hdlogs`                       | `tail -F ~/.config/herdr/herdr*.log`                               | Follow Herdr logs                                                                                |
| `zh`                           | `zellij-herdr`                                                     | Attach/create the Zellij Herdr session with the generated `herdr.kdl` layout                    |
...
| `aip`                          | AI Panes — multi-agent side-by-side in Herdr or Zellij             | Function: `aip cl oc gem "prompt"` opens panes with prompt and names the tab                     |
...
- **Aliases** (`cl`, `ocl`, `hcl`, `clglm`, `oc`, `ocglm`, `ocgem`, `ocgpt`, `locgpt`, `mocgpt`, `xocgpt`, `ocs`, `oczen`, `opi`, `gem`, `cx`, `lcx`, `mcx`, `hcx`, `xcx`, `hd`, `hds`, `hdi`, `hdagents`, `hdlogs`, `zh`, `ais`, `ait`, `occm`, `ocbp`, `ocor`, `oc-port`, `ai-mcp-scan`, etc.) are generated by `helpers/_aliases.nix` and merged into shell scope via `helpers/_services-shell-aliases.nix`. Workflow aliases (`*cm`, `*rf`, `*fx`, `*sa`, `*du`, `*bp`, `*rp`, `*md`) and clipboard prompt aliases (`cp*`: `cpcm`, `cprf`, `cpfx`, `cpsa`, `cpdu`, `cpbp`, `cprp`, `cpmd`) are generated automatically.
- **Packages** (`agents-search`, `iter`, `zellij-herdr`) are `writeShellScriptBin` wrappers added to `home.packages` in `home-manager/modules/ai-agents/packages.nix` and `home-manager/modules/terminal/scripts.nix`.
...
- When launched from inside Zellij, AI aliases/functions rename the focused tab to the launcher name (`ocglm`, `cl`, `gem`, `cx`, `occm`, etc.), and the status bar shows tab names instead of raw tab indexes.
- When launched from inside Herdr, `aip` uses Herdr's JSON API for tabs and panes instead of writing Zellij KDL.
...
Multi-agent parallel  -> aip cl oc gem "your task" (side-by-side in Herdr or Zellij)
Long agent workspace  -> hd or zh, then aip cl oc gem "your task"
```
